...
async function fetchCustomers(customerIds) {
  ...
  for (let customerId of customerIds) {
    ...
    for (let subscription of subscriptions) {
      subscriptionCancelledEvents.push(
  ...
  console.error(
    `Got ${subscriptionCancelledEvents.length} subscription cancelled events`
  );
...
function anonymiseData(data) {
  ...
  const subscriptionCancelledEvents = data.subscriptionCancelledEvents.map(
    anonymiseSubscriptionCancelledEvent
  );
