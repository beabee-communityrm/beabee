...
export type ExportResult = (Record<string, unknown>[])|{fields: string[], data: unknown[][]};
...
export default abstract class BaseExport<T> {
	...
	async getNewItemIds(): Promise<string[]> {
		return (await this.getNewItemsQuery().select(this.idColumn, 'id').getRawMany()).map(item => item.id);
	}
}
