  ...
  wrapAsync(
    hasUser(async function (req, res) {
      ...
      const otpoptions = querystring.stringify({
        issuer:
          (config.dev ? " [DEV] " : "") +
          OptionsService.getText("organisation"),
        secret: secret
      });
      const otpissuerName = encodeURIComponent(
        OptionsService.getText("organisation") + (config.dev ? "_dev" : "")
      );
      const otpauth =
        "otpauth://totp/" +
        otpissuerName +
  ...
  wrapAsync(
    hasUser(async function (req, res) {
      ...
      const hash = await hashPassword(
        req.body.password,
        req.user.password.salt,
        req.user.password.iterations
      );
