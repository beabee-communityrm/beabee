...
function groupsList(groups: string[]) {
	return groups.slice().sort((a, b) => a < b ? -1 : 1).join(',');
}

function isMismatchedMember(member: Member, nlMember: NewsletterMember) {
	return member.profile.newsletterStatus !== nlMember.status ||
...
async function handleResync(statusSource: 'ours'|'theirs') {
	try {
		...
		const newMembersToUpload = [], existingMembers = [], existingMembersToArchive = [], mismatchedMembers = [];
