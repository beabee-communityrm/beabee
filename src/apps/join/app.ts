...
app.get(
  ...
  wrapAsync(async function (req, res) {
    const referrer = await MembersService.findOne({
...
app.post(
  "/",
  [isNotLoggedIn, hasSchema(joinSchema).orFlash],
  wrapAsync(async function (req, res) {
...
async function handleJoin(
  ...
  res: Response,
  member: Member,
  { customerId, mandateId, joinForm }: CompletedJoinFlow
): Promise<void> {
  ...
  if (joinForm.referralCode) {
    const referrer = await MembersService.findOne({
      referralCode: joinForm.referralCode
    });
...
app.get(
  "/complete",
  [isNotLoggedIn, hasSchema(completeSchema).orRedirect("/join")],
  wrapAsync(async function (req, res) {
    ...
    const partialMember = await GCPaymentService.customerToMember(
      joinFlow.customerId
    );
    ...
    try {
      ...
      if (isDuplicateIndex(error, "email")) {
        const oldMember = await MembersService.findOne({
          email: partialMember.member.email
        });
        if (oldMember) {
          let redirectUrl = "";
          if (oldMember.isActiveMember) {
            ...
            redirectUrl = oldMember.activated
              ? "expired-member"
              : "confirm-email";
          }
...
app.get(
  ...
  wrapAsync(async (req, res, next) => {
    ...
    if (restartFlow) {
      if (
        restartFlow.member.isActiveMember ||
        !(await GCPaymentService.canChangeContribution(
          restartFlow.member,
          false
        ))
      ) {
